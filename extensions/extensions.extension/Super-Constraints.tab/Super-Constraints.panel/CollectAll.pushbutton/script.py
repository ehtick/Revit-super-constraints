#! python3
# collect elements from rooms and find design constraints
import clr
import os
import os.path
...
t = Transaction(doc, "Get all elements")
t.Start()

# collect all rooms in the building
room_collection = FilteredElementCollector(doc).OfClass(Room).ToElements()
...
for rm in room_collection:   
    sel_location = rm.Location.Point
    options = SpatialElementBoundaryOptions()
    calculator = SpatialElementGeometryCalculator(doc,options)
    results = calculator.CalculateSpatialElementGeometry(rm)
    ...
    boundingBox = rm.ClosedShell.GetBoundingBox()
    ...
    for id in added_elements:
        ...
        new_row = pd.Series({'Selected_roomI_uniqueId':rm.UniqueId,
                                        'Selected_roomI_Id': rm.Id,
